...
def plot_one_box(x, im, color=None, label=None, line_thickness=3):
    ...
    crop1 = im[int(x[1]):int(x[3]), int(x[0]):int(x[2])]
    #cv2.imwrite('C:\\Users\\anill\\Desktop\\projects\\number_plate\\data\\cropd.jpg',crop1)
    text1 = pytesseract.image_to_string(crop1, config='-l eng --oem 1 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 -c page_separator=')
    text = [t for t in text1.split('\n') if t not in ['', ' ', '\n', '\x0c']]
    if len(text):
        text=text[0]
    else:
        text=text1
    #-c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 -c page_separator=
    ...
    if label:
        ...
        #cv2.putText(im, label+'  '+text, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        #cv2.putText(im, text1, (50,50), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        
        c2 = c1[0] + t_size[0], c1[1] - 2*t_size[1] - 3
        cv2.rectangle(im, c1, c2, color, -1, cv2.LINE_AA)  # filled
        
        cv2.putText(im, text, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        c1 = c1[0] , c1[1]-t_size[1]-3
        #c1 = c1[0] , c1[1]+t_size[1]+3
